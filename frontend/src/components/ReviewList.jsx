...
function ReviewList({ productId, editingReview, setEditingReview }) {
        ...
        {reviews.map((review) => (
          <div key={review.id} className="border rounded-xl p-6">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-bold text-lg">{review.user.full_name}</h3>
              ...
              {user?.id === review.user.id && (
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => removeReview(review.id, productId)}
            ...
            <p className="mt-6 text-gray-700 leading-8">{review.comment}</p>
